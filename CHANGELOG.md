...
### 修复
- **HLS 播放回归**：部分 FLV 文件在 FFmpeg 已生成完整 `#EXT-X-ENDLIST` 播放列表和分片后，因文件尾部坏帧以非零退出码结束；此前会被错误返回为 HTTP 503，导致鸿蒙端无法播放。现在仅在播放列表不完整时判定 HLS 转封装失败。

### 保留
- 保留 v2.1.0 的 HLS 缓存启动清理、TTL/总量回收、锁生命周期管理、独立 30 分钟转封装超时，以及正在录制文件拒绝生成 HLS 的行为。
